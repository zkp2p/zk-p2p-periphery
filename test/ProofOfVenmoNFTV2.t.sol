...
import { ProofOfVenmoNFTV2 } from "../src/ProofOfVenmoNFTV2.sol";

contract ProofOfVenmoNFTV2Test is Test {
    ProofOfVenmoNFTV2 public proofOfVenmoNFTV2;
    ...
    function setUp() public {
        ...
        proofOfVenmoNFTV2 = new ProofOfVenmoNFTV2(ramp);
    }
    
    function test_Mint() public {
        uint256 prevMinted = proofOfVenmoNFTV2.getTokenId(address(1));
        assertEq(prevMinted, 0);

        vm.startPrank(address(1));
        proofOfVenmoNFTV2.mintSBT();
        vm.stopPrank();
        
        uint256 tokenId = proofOfVenmoNFTV2.currentTokenId();
        assertEq(tokenId, 1);

        address owner = proofOfVenmoNFTV2.ownerOf(tokenId);
        assertEq(owner, address(1));

        uint256 balance = proofOfVenmoNFTV2.balanceOf(address(1));
        assertEq(balance, 1);

        uint256 currMinted = proofOfVenmoNFTV2.getTokenId(address(1));
        assertEq(currMinted, 1);

        string memory tokenURI = proofOfVenmoNFTV2.tokenURI(tokenId);
    ...
    function test_MintTwoSameIdHash() public {
        ...
        uint256 currTokenId = proofOfVenmoNFTV2.currentTokenId();
        assertEq(currTokenId, 2);

        address ownerOne = proofOfVenmoNFTV2.ownerOf(1);
        assertEq(ownerOne, address(1));

        uint256 balanceOne = proofOfVenmoNFTV2.balanceOf(address(1));
        assertEq(balanceOne, 1);

        uint256 mintedOne = proofOfVenmoNFTV2.getTokenId(address(1));
        assertEq(mintedOne, 1);

        address ownerTwo = proofOfVenmoNFTV2.ownerOf(2);
        assertEq(ownerTwo, address(2));

        uint256 balanceTwo = proofOfVenmoNFTV2.balanceOf(address(2));
        assertEq(balanceTwo, 1);

        uint256 mintedTwo = proofOfVenmoNFTV2.getTokenId(address(2));
    ...
    function test_RevertTransfers() public {
        ...
        proofOfVenmoNFTV2.transferFrom(address(1), address(2), 1);

        vm.expectRevert("ERC721 public safeTransferFrom not allowed");
        proofOfVenmoNFTV2.safeTransferFrom(address(1), address(2), 1);
