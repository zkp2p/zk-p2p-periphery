...
import { ProofOfP2PNFTV2 } from "../src/ProofOfP2PNFTV2.sol";

contract ProofOfP2PNFTV2Test is Test {
    ProofOfP2PNFTV2 public proofOfP2PNFTV2;
    ...
    function setUp() public {
        ...
        proofOfP2PNFTV2 = new ProofOfP2PNFTV2(
            ramp,
            "Proof of UPI-V1",
            "PoUPI-V1",
            "UPI",
    ...
    function test_Mint() public {
        uint256 prevMinted = proofOfP2PNFTV2.getTokenId(address(1));
        assertEq(prevMinted, 0);

        vm.startPrank(address(1));
        proofOfP2PNFTV2.mintSBT();
        vm.stopPrank();
        
        uint256 tokenId = proofOfP2PNFTV2.currentTokenId();
        assertEq(tokenId, 1);

        address owner = proofOfP2PNFTV2.ownerOf(tokenId);
        assertEq(owner, address(1));

        uint256 balance = proofOfP2PNFTV2.balanceOf(address(1));
        assertEq(balance, 1);

        uint256 currMinted = proofOfP2PNFTV2.getTokenId(address(1));
        assertEq(currMinted, 1);

        string memory tokenURI = proofOfP2PNFTV2.tokenURI(tokenId);
    ...
    function test_MintTwoSameIdHash() public {
        ...
        uint256 currTokenId = proofOfP2PNFTV2.currentTokenId();
        assertEq(currTokenId, 2);

        address ownerOne = proofOfP2PNFTV2.ownerOf(1);
        assertEq(ownerOne, address(1));

        uint256 balanceOne = proofOfP2PNFTV2.balanceOf(address(1));
        assertEq(balanceOne, 1);

        uint256 mintedOne = proofOfP2PNFTV2.getTokenId(address(1));
        assertEq(mintedOne, 1);

        address ownerTwo = proofOfP2PNFTV2.ownerOf(2);
        assertEq(ownerTwo, address(2));

        uint256 balanceTwo = proofOfP2PNFTV2.balanceOf(address(2));
        assertEq(balanceTwo, 1);

        uint256 mintedTwo = proofOfP2PNFTV2.getTokenId(address(2));
    ...
    function test_RevertTransfers() public {
        ...
        proofOfP2PNFTV2.transferFrom(address(1), address(2), 1);

        vm.expectRevert("ERC721 public safeTransferFrom not allowed");
        proofOfP2PNFTV2.safeTransferFrom(address(1), address(2), 1);
